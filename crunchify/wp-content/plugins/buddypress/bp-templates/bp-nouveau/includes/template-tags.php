...
function bp_nouveau_signup_form( $section = 'account_details' ) {
	...
	foreach ( $fields as $name => $attributes ) {
		if ( 'signup_password' === $name ) {
			...
			list( $label, $required, $value, $attribute_type, $type, $class, $placeholder ) = array_values( $attributes );
			...
			$field_output = sprintf(
				'<input type="%1$s" name="%2$s" id="%3$s" %4$s value="%5$s" %6$s placeholder="%7$s" />',
				esc_attr( $type ),
				esc_attr( $name ),
				esc_attr( $id ),
